...
func bindingInfo() -> [String: Any] {
    [
        "protection": "mac-keychain",
        "exportable": false,
        "attestation_supported": false,
        "device_binding": "platform:keychain",
    ]
}

func keyTag(_ name: String) -> Data {
    "com.unolock.agent.keychain.\(name)".data(using: .utf8)!
}

func secretService(_ name: String) -> String {
    "com.unolock.agent.keychain.secret.\(name)"
}
...
func createKey(_ name: String) throws -> [String: Any] {
    ...
    guard let publicKey = SecKeyCopyPublicKey(privateKey) else {
        throw NSError(domain: "UnoLockMacKeychain", code: 1, userInfo: [NSLocalizedDescriptionKey: "missing_public_key"])
    }
...
func sign(_ name: String, _ challengeB64: String) throws -> [String: Any] {
    ...
    guard status == errSecSuccess, let privateKey = item as! SecKey? else {
        throw NSError(domain: "UnoLockMacKeychain", code: Int(status), userInfo: [NSLocalizedDescriptionKey: "key_not_found"])
    }
    guard let challenge = Data(base64Encoded: challengeB64) else {
        throw NSError(domain: "UnoLockMacKeychain", code: 2, userInfo: [NSLocalizedDescriptionKey: "invalid_challenge_b64"])
    }
    var error: Unmanaged<CFError>?
...
func storeSecret(_ name: String, _ secretB64: String) throws -> [String: Any] {
    guard let secret = Data(base64Encoded: secretB64) else {
        throw NSError(domain: "UnoLockMacKeychain", code: 6, userInfo: [NSLocalizedDescriptionKey: "invalid_secret_b64"])
    }
    let query: [String: Any] = [
    ...
    guard status == errSecSuccess else {
        throw NSError(domain: "UnoLockMacKeychain", code: Int(status), userInfo: [NSLocalizedDescriptionKey: "secret_store_failed"])
    }
    return [
        "ok": true,
        "stored": name,
...
func loadSecret(_ name: String) throws -> [String: Any] {
    ...
    guard status == errSecSuccess, let data = item as? Data else {
        throw NSError(domain: "UnoLockMacKeychain", code: Int(status), userInfo: [NSLocalizedDescriptionKey: "secret_load_failed"])
    }
    return [
        "ok": true,
...
func diagnose() throws -> [String: Any] {
    ...
    return [
        ...
        "summary": "macOS Keychain created a non-exportable P-256 key.",
        "binding_info": bindingInfo(),
        "public_key_b64": created["public_key_b64"]!,
    ]
}
...
do {
    ...
    case "create-key":
        guard args.count >= 3 else {
            throw NSError(domain: "UnoLockMacKeychain", code: 3, userInfo: [NSLocalizedDescriptionKey: "missing_key_name"])
        }
    ...
    case "sign":
        guard args.count >= 4 else {
            throw NSError(domain: "UnoLockMacKeychain", code: 4, userInfo: [NSLocalizedDescriptionKey: "missing_sign_args"])
        }
    ...
    case "store-secret":
        guard args.count >= 4 else {
            throw NSError(domain: "UnoLockMacKeychain", code: 7, userInfo: [NSLocalizedDescriptionKey: "missing_secret_args"])
        }
    ...
    default:
        throw NSError(domain: "UnoLockMacKeychain", code: 5, userInfo: [NSLocalizedDescriptionKey: "unknown_action"])
    }
} catch {
    jsonOut([
        "ok": false,
...
class MacKeychainDao(TpmDao):
    ...
    def diagnose(self) -> TpmDiagnostics:
        ...
        if platform.system().lower() != "darwin":
            advice.append("The macOS Keychain provider is only available on macOS.")
            return TpmDiagnostics(
                provider_name=self.provider_name(),
                provider_type="platform",
                production_ready=False,
                available=False,
                summary="macOS Keychain is not available on this host.",
                details=details,
                advice=advice,
            )
        if not self._swift:
            advice.append("Install Xcode command line tools so the MCP can run the macOS Keychain helper.")
            return TpmDiagnostics(
                provider_name=self.provider_name(),
                provider_type="platform",
                production_ready=False,
        ...
        try:
            ...
            advice.append("This provider uses a non-exportable key stored in the user's macOS Keychain.")
            advice.append("Prefer Secure Enclave when it works cleanly, but this path is acceptable for broader macOS compatibility.")
            return TpmDiagnostics(
                provider_name=self.provider_name(),
                provider_type="platform",
                production_ready=True,
                available=True,
                summary=str(payload.get("summary", "macOS Keychain provider is available.")),
                details=details,
                advice=advice,
            )
    ...
    def _run_helper(
        ...
        *,
        ...
        if helper_binary:
            ...
            proc = subprocess.run(command, check=False, capture_output=True, text=True, timeout=20)
            stdout = proc.stdout.strip()
            stderr = proc.stderr.strip()
            if not stdout:
                raise RuntimeError(stderr or "macOS Keychain helper returned no output")
            payload = json.loads(stdout)
            if proc.returncode != 0 or not payload.get("ok"):
                raise RuntimeError(str(payload.get("error") or stderr or "macOS Keychain helper failed"))
            return payload

        with tempfile.NamedTemporaryFile("w", suffix=".swift", delete=False, encoding="utf8") as handle:
            script_path = Path(handle.name)
            handle.write(MACOS_KEYCHAIN_HELPER)
        try:
    ...
    def _ensure_compiled_helper(self) -> Path | None:
        ...
        helper_binary = cache_dir / f"mac-keychain-helper-{self._helper_hash()}"
        if helper_binary.exists():
            return helper_binary
        source_path = cache_dir / f"{helper_binary.name}.swift"
        source_path.write_text(MACOS_KEYCHAIN_HELPER, encoding="utf8")
        try:
            compile_command = self._build_compile_command(source_path, helper_binary)
            proc = subprocess.run(compile_command, check=False, capture_output=True, text=True, timeout=60)
            if proc.returncode != 0:
                stderr = proc.stderr.strip()
                raise RuntimeError(stderr or "macOS Keychain helper compilation failed")
    ...
    def _binding_info_from_payload(self, payload: object) -> KeyBindingInfo:
        if not isinstance(payload, dict):
            raise TypeError("Invalid binding info payload from macOS Keychain helper")
        return KeyBindingInfo(
            protection=str(payload.get("protection", "mac-keychain")),
            exportable=bool(payload.get("exportable", False)),
            attestation_supported=bool(payload.get("attestation_supported", False)),
            device_binding=str(payload.get("device_binding", "platform:keychain")),
        )
    ...
    def _diagnostic_summary_for_exception(exc: Exception) -> str:
        return "macOS Keychain helper could not create a signing key."
