...
function New-BindingInfo() {
  return [ordered]@{
    protection = 'windows-cng'
    exportable = $false
    attestation_supported = $false
    device_binding = 'platform:windows-cng'
  }
}
  ...
  switch ($Action) {
    'diagnose' {
      ...
      To-JsonOut([ordered]@{
        ...
        summary = 'Windows CNG created a non-exportable P-256 key in the software key storage provider.'
...
class WindowsCngDao(TpmDao):
    ...
    def diagnose(self) -> TpmDiagnostics:
        ...
        if not self._powershell:
            advice.append("Install or expose powershell.exe so the Windows CNG helper can run.")
        ...
        try:
            ...
            if _is_wsl():
                advice.append("WSL can use the Windows software key storage provider through the Windows helper.")
            advice.append("This provider uses a non-exportable Windows CNG key when TPM-backed creation is unavailable.")
            return TpmDiagnostics(
                provider_name=self.provider_name(),
                provider_type="platform",
                production_ready=True,
                available=True,
                summary=str(payload.get("summary", "Windows CNG provider is available.")),
                details=details,
                advice=advice,
            )
            ...
            return TpmDiagnostics(
                ...
                production_ready=False,
                available=False,
                summary="Windows CNG helper could not create a non-exportable key.",
                details=details,
                advice=advice,
            )
    ...
    def _run_helper(
        ...
        *,
        ...
        with tempfile.NamedTemporaryFile("w", suffix=".ps1", delete=False, encoding="utf8") as handle:
            ...
            handle.write(WINDOWS_CNG_HELPER)
        try:
            command = [
                self._powershell,
                "-NoProfile",
            ...
            if not stdout:
                raise RuntimeError(stderr or "Windows CNG helper returned no output")
            payload = json.loads(stdout)
            if proc.returncode != 0 or not payload.get("ok"):
                raise RuntimeError(str(payload.get("error") or stderr or "Windows CNG helper failed"))
            return payload
        finally:
    ...
    def _binding_info_from_payload(self, payload: object) -> KeyBindingInfo:
        if not isinstance(payload, dict):
            raise TypeError("Invalid binding info payload from Windows CNG helper")
        return KeyBindingInfo(
            protection=str(payload.get("protection", "windows-cng")),
            exportable=bool(payload.get("exportable", False)),
            attestation_supported=bool(payload.get("attestation_supported", False)),
            device_binding=str(payload.get("device_binding", "platform:windows-cng")),
        )
...
def _is_wsl() -> bool:
    return platform.system().lower() == "linux" and ("microsoft" in platform.release().lower() or "WSL_INTEROP" in os.environ)
